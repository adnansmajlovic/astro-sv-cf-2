...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    const authInspection = {
      ...
      methods: auth
        ? Object.getOwnPropertyNames(Object.getPrototypeOf(auth)).filter(
            (name) => typeof auth[name] === "function",
          )
        : [],
      handler: {
        exists: !!auth?.handler,
    ...
    if (!auth?.handler) {
      ...
      diagnosis.recommendations.push(
        "Verify Better Auth import and initialization",
      );
    }

    if (
      !envValidation.googleClientId.set ||
      !envValidation.googleClientSecret.set
    ) {
    ...
    if (!dbValidation.hasInstance) {
      ...
      diagnosis.recommendations.push(
        "Check database configuration in middleware",
      );
    }

    const missingTables = dbValidation.tables.filter(
    ...
    if (failedEndpoints.length > 0) {
      diagnosis.issues.push(
        `${failedEndpoints.length} auth endpoints returning 404`,
      );

      // Check specific patterns
      if (failedEndpoints.find((e) => e.path === "/api/auth/session")) {
        diagnosis.recommendations.push(
          "Basic session endpoint not working - check Better Auth initialization",
        );
      }

      if (failedEndpoints.find((e) => e.path.includes("email"))) {
        diagnosis.recommendations.push(
          "Email auth endpoints missing - add emailAndPassword: { enabled: true } to auth config",
        );
      }
    }
    ...
    if (
      workingEndpoints.length === 1 &&
      workingEndpoints[0].path === "/api/auth/callback/google"
    ) {
      diagnosis.issues.push(
        "Only OAuth callback working - auth not fully initialized",
      );
      diagnosis.recommendations.push(
        "Check if auth.handler is properly receiving and processing requests",
      );
      diagnosis.recommendations.push(
        "Verify database adapter is correctly configured",
      );
    }
